...
def test_snapshot_is_the_extended_format_the_gate_resolves(tmp_path, monkeypatch, capsys):
    ...
    assert document["schema"] == "cathedral_registration_snapshot_v2"
    assert document["block"] == 42
    assert document["block_is_finalized"] is True
...
def test_snapshot_builder_rejects_an_invalid_audience_or_block(
    ...
    with pytest.raises(SystemExit):
        allowlist_tool.build_snapshot_document(
            [(OPERATOR_HOTKEY, OPERATOR_COLDKEY)],
            network=network,
            netuid=netuid,
            block=block,
        )
